...
int setup_f_query_join(Query *query, int argc, char *argv[], int qname_idx);

template<class QueryBase>
class FQueryJoinSampling: public QueryBase {
public:
	FQueryJoinSampling()
	...
	virtual int additional_setup(int argc, char *argv[], int qname_idx) {
		if (argc <= qname_idx + 7) {
			print_f_query_join_help(this, argv, qname_idx);
			return 1;
		}
